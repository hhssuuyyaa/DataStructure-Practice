...
void elementsAtDepth(node* root, int d,int k){
    if(root==NULL){
        return;
    }
    elementsAtDepth(root->left,d+1,k);
    elementsAtDepth(root->right,d+1,k);
    if(d==k){
        cout<<root->data<<" ";
    }
    return;
}
//practice and imagine like utkarsh bhaiya do. Make diag and think recursion as magic. and just handle root condition. Rest will work
//No rocket science is happening
bool isNodePresent(node* root,int k){
    if(root==NULL){
        return false;
    }
    bool left = isNodePresent(root->left,k);
    bool right = isNodePresent(root->right,k);
    if(root->data == k || left || right){
        return true;
    }else{
        return false;
    }
